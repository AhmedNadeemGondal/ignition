...
const getBaseURL = () =>
  isDevelopment ? base_url : `${server_url}${encodeURIComponent(base_url)}`;

export const popGURL = () => `${getBaseURL()}${pop_games}`;
export const upComeGURL = () => `${getBaseURL()}${upcome_games}`;
export const newGURL = () => `${getBaseURL()}${new_games}`;

export const pop_games_page = (page) => {
  console.log("This fired!");
  return `${getBaseURL()}games?${local_key}&dates=${lastYear},${currDate}&ordering=-rating&page_size=10&page=${page}`;
};

export const upcome_games_page = (page) => {
  console.log("This fired!");
  return `${getBaseURL()}games?${local_key}&dates=${lastYear},${currDate}&ordering=-added&page_size=10&page=${page}`;
};

export const new_games_page = (page) => {
  console.log("This fired!");
  return `${getBaseURL()}games?${local_key}&dates=${lastYear},${currDate}&ordering=-released&page_size=10&page=${page}`;
};

export const gameDetailURL = (game_id) =>
  `${getBaseURL()}games/${game_id}?${local_key}`;

export const gameScreenShotURL = (game_id) =>
  `${getBaseURL()}games/${game_id}/screenshots?${local_key}`;

export const searchGameURL = (game_name) =>
  `${getBaseURL()}games?${local_key}&search=${game_name}&page_size=9`;

export const search_games_page = (game_name, page) => {
  console.log("This fired!");
  return `${getBaseURL()}games?${local_key}&search=${game_name}&page_size=9&page=${page}`;
};
